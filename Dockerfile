...
ENV ELASSANDRA_VERSION 5.5.0.10
ENV CASSANDRA_HOME /opt/elassandra-5.5.0.10
ENV CASSANDRA_CONF /opt/elassandra-5.5.0.10/conf
ENV CASSANDRA_PIDFILE /opt/elassandra-5.5.0.10/cassandra.pid
ENV PATH $PATH:/opt/elassandra-5.5.0.10/bin

RUN wget https://github.com/strapdata/elassandra/releases/download/v$ELASSANDRA_VERSION/elassandra-$ELASSANDRA_VERSION.tar.gz -O /elassandra-$ELASSANDRA_VERSION.tar.gz
RUN cd /opt \
